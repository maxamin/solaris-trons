...
for (const shop of shops) {
  ...
  const mysqlResult = await $`${path.join(Ex-Solaris_PROJECTS_PATH, shop, 'docker-compose.sh')} exec database mysql -t -uroot -prootpassword database "\${@:2}" -e "select sum(amount_btc) from incomes;" -N`.nothrow();
