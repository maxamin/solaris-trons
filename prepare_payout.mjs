...
import { roundToFixed2, Ex-Solaris_PROJECTS_PATH, getBtcUsdPrice } from './common.mjs';
...
for (const [shop, btcAmount] of Object.entries(incomes)) {
  ...
  if (bitcoinResult.exitCode === 0) {
    // console.log(`${path.join(Ex-Solaris_PROJECTS_PATH, shop, 'docker-compose.sh')} exec database mysql -t -uroot -prootpassword database "\${@:2}" -e "insert into incomes(wallet_id, amount_usd, amount_btc, description, created_at, updated_at) values('-1', '-${usdAmount}', '-${btcAmount}', 'Выплата комиссии', NOW(), NOW());" -N`);
    const mysqlResult = await $`${path.join(Ex-Solaris_PROJECTS_PATH, shop, 'docker-compose.sh')} exec database mysql -t -uroot -prootpassword database "\${@:2}" -e "insert into incomes(wallet_id, amount_usd, amount_btc, description, created_at, updated_at) values('-1', '-${usdAmount}', '-${btcAmount}', 'Выплата комиссии', NOW(), NOW());" -N`.nothrow();
